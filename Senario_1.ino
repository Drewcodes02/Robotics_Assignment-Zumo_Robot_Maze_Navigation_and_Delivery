...
unsigned int deliveryCount = 0; // to count how mant deliveries its made
...
void mazeFunctionalities() {
  ...
  if (deliveryCount >= 1) { // if delivery has been made it stops the zumo and any further actions
    return; // stops zumo
  }
...
// 180 turn every 5 seconds incase the zumo gets stuck
  ...
  // object detection logic
    ...
    if (objectDetected) {
        ...
        deliveryCount++; // checks if delivery has been completed
        return; // stops zumo if made delivery as no further action is needed

        } else if (currentTime - lastTurnTime >= 10000) { // checks for 180 degree turns
        buzzer.playFrequency(880, 500, 15); // beeps before turn
        delay(500);
        perform180Turn(); // 
        lastTurnTime = millis();
    } else {
        mazeNavigation(); // continues navigating through the maze if no object is detected
    }
}
